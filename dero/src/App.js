...
import {Deposit, Stake} from './components/Client.js';
import { getActiveAccount, getContract, getContractStorage, wallet } from './tezos.js';
...
function App() {
  ...
  const [chipCount, setChipCount] = useState(10000000); 
  ...
  async function initialize() {
		...
		const amount = storage.playerBook.get(account.address);
		
		console.log(amount.depositAmt.toString());
		const deposit = Number(amount.depositAmt.toString());
		console.log(deposit);
		console.log(account.address);
		console.log(chipCount);
		setChipCount(deposit); 
		console.log(chipCount);		//need to change
	  }; 
  ...
  const spinTheWheel = async () => {
    if (!isSpinComplete && pendingTotalBet > 0 && !isWheelSpinning) {
		...
		const op = await contract.methods.betting(pendingTotalBet).send();
  ...
  const collectWinnings = async () => {
    ...
    if (totalAmountWon > 0) {
		...
		//const address = account.address;
		const contract = await getContract();
		const op = await contract.methods.winnings(totalAmountWon).send();
		await op.confirmation();
  ...
  return (
    <div className="app">
      <header className="app-header">
        <div className="header-div left-header">
         <h3 className="header-h3">DeRo</h3>
        </div>
